  ...
  React.useEffect(() => {
    ...
    axios
      ...
      .then((res) => {
        ...
        setFilteredProducts(res.data); // Initially, set filtered products same as all products
      })
      .catch((error) => {
        ...
        <Toolbar>
          ...
          <Search>
            ...
            <ul
              ...
              className="lg:w-96 sm:w-96 md:w-96 w-64"
            >
              {hide &&
                filteredProducts?.slice(0, 8)?.map((product) => (
                  <>
                    <li className="button" key={product.id} style={{ padding: "8px" }} onClick={(e)=>{router.push(`/detailPage/${product.id}`);localStorage.setItem('id',product.id);setHide(false);textRef.current.value="";}}>
                      {product.title}
                    </li>
                    <Divider />
                  </>
